...
\begin{enumerate}
  ...
  \item \textbf{Typed inter-language state.} \ovalue{} gives every evaluator a
        common algebra for values, requests, groups, scopes, and capabilities;
        separate judgments describe modeled value crossings and renderer
        projections.
...
language regions. \\
Typed exchange &
\ovalue{}, canonical CBOR, renderer contracts, and boundary-specific fidelity
judgments. &
The exchange boundary preserves tagged meaning; modeled value crossings and
renderer projections receive separate judgments. \\
Graph semantics &
...
coordinate, backend Catalog V6 projection, backend artifact bindings, runtime
...
and source rendering retain distinct boundary-specific fidelity judgments;
value-crossing judgments expose the public cross-boundary composition
operation, while render-fidelity combination is confined to recursive child
folding within one renderer.}
...
\texttt{kind=value} with \texttt{value}, or \texttt{kind=ref} with
\texttt{target}. \\
...
\texttt{NativeCapsule}, or \texttt{Unsupported}. Composition unions structural
loss sets and selects the absorbing severity classes; its idempotence permits
the HGraph solver to converge at a fixpoint.
\src{crates/ostadix-api/src/value.rs}{587--685}

\texttt{FidelityAssessmentV2} refines the structural case to an interval of
definite and possible loss sets, with the checked invariant
$D \subseteq P$. Its \texttt{from\_concrete} operation is an exact point
embedding of a V1 judgment; concretization is exposed as a membership test
rather than an exponential powerset. Sequential composition unions both
bounds. The structural/lossless path join is the interval hull, while
\texttt{NativeCapsule} and \texttt{Unsupported} remain absorbing severity
classes rather than cross-class disjunctions. These implemented laws do not by
themselves establish a Galois connection.
\src{crates/ostadix-api/src/value.rs}{234--585}

\texttt{RenderFidelity} can be recomputed on demand as a separate description
of source projection. It combines classifications only while recursively
folding children within one renderer. No conversion, common carrier, or Galois
connection relates it to \texttt{Fidelity} or \texttt{FidelityAssessmentV2}.
\src{crates/ostadix-api/src/eval_core.rs}{250--481}
...
first bound each profile to its canonical backend entry, and Catalog V6 retains
those assignments. The HGraph solver exposes
...
\src{crates/ostadix-api/src/backend_morphism.rs}{126--330}
\src{crates/ostadix-api/src/backend_morphism.rs}{332--499}
\src{crates/ostadix-api/src/hgraph/solve.rs}{866--912}
...
\begin{itemize}
  ...
  \item A rich typed interchange algebra, separate explicit crossing-fidelity
        assessments, and a canonical CBOR wire protocol capped at 128 MiB per
        frame.

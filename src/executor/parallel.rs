...
pub(crate) fn render_inputs_pure(frame: &GraphEvalFrame, plan: &ExecutionPlan, id: PlanNodeId) -> bool {
...
pub(crate) fn build_task(
    ...
    for child in children {
        match &plan.nodes[child.0].kind {
            ...
            _ => {
                parts.push(RenderPart::Splice(frame.value(child)?.clone()));
            }
        }
    }
    Ok(ParallelTask {
...
pub(crate) fn execute(tasks: Vec<ParallelTask>) -> Vec<Result<OValue>> {
    ...
    std::thread::scope(|scope| {
        ...
        for (slot, handle) in results.iter_mut().zip(handles.into_iter()) {

        ...
        let (kind, ty) = match &expr.kind {
            ...
            ast::ExprKind::Binary { op, lhs, rhs } => {
                ...
                let rhs_expected =
                    ...
                    } else if matches!(op, ast::BinaryOp::ShiftLeft | ast::BinaryOp::ShiftRight) {
                ...
                let result_ty = match op {
                    ...
                    ast::BinaryOp::ShiftLeft | ast::BinaryOp::ShiftRight => {
                        if !self.program.types.is_integer(lhs.ty) {
                            return Err(self.error(lhs.span, "shifted value must be an integer"));
                        }
                        if !self.program.types.is_integer(rhs.ty) {
                            return Err(self.error(rhs.span, "shift count must be an integer"));
                        }
                        lhs.ty
                    }
                    _ => {

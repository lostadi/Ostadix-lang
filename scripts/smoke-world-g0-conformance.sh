...
printf '%s\n' \
    ...
    "artifact:world-contract-v1:sha256=$base_contract_digest" \
    "artifact:world-contract-v2:sha256=$contract_digest" \
    "artifact:o-machine-contract-v1:sha256=$machine_contract_digest"

python3 scripts/world_alpha_evidence.py --definitions-only --quiet
printf '%s\n' 'G0 executable contract schema: PASS'
printf '%s\n' 'G0 executable O-Machine contract schema: PASS'

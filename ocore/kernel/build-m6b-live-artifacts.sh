...
BUILD_DIR="${OCORE_M6B_LIVE_BUILD_DIR:-$ROOT/target/ocore-m6b-live-artifacts}"
...
if ! command -v objdump >/dev/null 2>&1; then
  echo "error: objdump is required for strict M6B-live ELF inspection" >&2
  exit 127
fi
if ! command -v nm >/dev/null 2>&1; then
  echo "error: nm is required for M6B-live entry-symbol inspection" >&2
  exit 127
fi
...
CLIENT_SOURCE="$ROOT/ocore/runtime/x86_64/m6b_live_client.oc"
DAEMON_SOURCE="$ROOT/ocore/runtime/x86_64/m6b_live_personalityd.oc"
SUPERVISOR_SOURCE="$ROOT/ocore/runtime/x86_64/m6b_live_supervisord.oc"
if [[ "$(grep -Fc 'syscall6(' "$CLIENT_SOURCE")" != 1 ]] \
    || ! grep -Fq 'const SYS_PERSONALITY_BOUNDED_CALL: u64 = 17;' "$CLIENT_SOURCE" \
    || ! grep -Fq 'if result == ERR_IPC_RETRY' "$CLIENT_SOURCE" \
    || ! grep -Fq ') != ERR_PERSONALITY_FAILED' "$CLIENT_SOURCE" \
    || ! grep -Fq ') != ERR_PERSONALITY_BUSY' "$CLIENT_SOURCE" \
    || ! grep -Fq 'const SYS_PERSONALITY_VIEW_LOOKUP: u64 = 18;' "$DAEMON_SOURCE" \
    || ! grep -Fq 'const SYS_PERSONALITY_VIEW_READ: u64 = 19;' "$DAEMON_SOURCE" \
    || ! grep -Fq 'const SYS_PERSONALITY_VIEW_WRITE: u64 = 20;' "$DAEMON_SOURCE" \
    || ! grep -Fq 'const SYS_PERSONALITY_BOUNDED_REPLY: u64 = 21;' "$DAEMON_SOURCE" \
    || ! grep -Fq 'const OP_HOLD_UNMAP: u64 = 6;' "$CLIENT_SOURCE" \
    || ! grep -Fq 'const OP_HOLD_REQUEST_REVOKE: u64 = 7;' "$CLIENT_SOURCE" \
    || ! grep -Fq 'const OP_HOLD_RESOURCE_REVOKE: u64 = 8;' "$CLIENT_SOURCE" \
    || ! grep -Fq 'const OP_HOLD_PROCESS_EXIT: u64 = 9;' "$CLIENT_SOURCE" \
    || ! grep -Fq 'progress(11);' "$CLIENT_SOURCE" \
    || [[ "$(grep -Fc 'administer(supervise_capability' "$SUPERVISOR_SOURCE")" != 4 ]]; then
  echo "error: M6B-live packaged syscall/no-reissue source contract drift" >&2
  exit 1
fi
...
link_service() {
  ...
  local service="$2"
  "$OCOREC" \
    "$ROOT/ocore/runtime/x86_64/m6b_live_${service}.oc" \
...
check_elf() {
  ...
  if ! grep -Fq 'file format elf64-x86-64' <<<"$file_headers" \
      ...
      || grep -aFq 'M6A ' "$elf" \
      || ! grep -aFq 'M6B live' "$elf"; then
    echo "error: noncanonical or mislabeled M6B-live static ELF: $elf" >&2
    printf '%s\n%s\n%s\n' "$file_headers" "$program_headers" "$section_headers" >&2
    exit 1
  fi
  if [[ "$service" == client || "$service" == supervisord ]]; then
    if [[ "$load_count" != 1 || "$flags" != $'r-x\nrw-' || -n "$rodata_vma" ]]; then
      echo "error: M6B-live payload must be the exact RX-only profile: $elf" >&2
      exit 1
    fi
  else
    if [[ "$load_count" != 2 \
      ...
      echo "error: M6B-live payload segment profile mismatch: $elf" >&2
      exit 1
    fi
  fi
  case "$service" in
    client)
      grep -aFq 'M6B live direct saved-RAX result/no reissue: PASS' "$elf"
      ;;
    personalityd)
      grep -aFq 'M6B live delegated view close/copy denial: PASS' "$elf"
      ;;
    supervisord)
      grep -aFq 'M6B live supervisor policy loop: PASS' "$elf"
      ;;
    observer)
      grep -aFq 'M6B live unrelated observer ELF: online' "$elf"
      ;;
  esac
}
...
verify_image() {
  ...
  if ! grep -Eq '^OVFSIMG1 verified: 4 files, [0-9]+ bytes, sha256=[0-9a-f]{64}$' <<<"$verification" \
    ...
    echo "error: M6B-live OVFS does not contain the exact four-service corpus" >&2
    printf '%s\n' "$verification" >&2
    exit 1
  fi
}
...
for service in client personalityd supervisord observer; do
  check_elf "$PASS_ONE/${service}.elf" "$service"
  check_elf "$PASS_TWO/${service}.elf" "$service"
...
if [[ "$DIGEST_ONE" != "$DIGEST_TWO" ]] || ! cmp -s "$IMAGE_ONE" "$IMAGE_TWO"; then
  echo "error: M6B-live OVFSIMG1 rebuild was not deterministic" >&2
  exit 1
fi
...
file \
  ...
  "$IMAGE_ONE"
printf 'M6B live artifact build: PASS\n'
for service in client personalityd supervisord observer; do

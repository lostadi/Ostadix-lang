...
BUILD_DIR="${OCORE_BUILD_DIR:-$ROOT/target/ocore-world-protocol}"
FIXTURE="$ROOT/tests/fixtures/world_protocol_v1.hex"
TIMEOUT_SECONDS=30

for tool in cargo qemu-system-x86_64 python3; do
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "error: $tool is required for the World protocol smoke" >&2
    exit 127
  fi
done
if [[ ! -f "$FIXTURE" ]]; then
  echo "error: missing World protocol byte oracle: $FIXTURE" >&2
  exit 1
fi
...
if not expected_hex or len(expected_hex) % 2 or re.fullmatch(r"[0-9a-f]+", expected_hex) is None:
    print("World protocol fixture is not canonical lowercase even-length hex", file=sys.stderr)
    raise SystemExit(1)
if len(expected_hex) // 2 != 1254:
    print("World protocol fixture is not the pinned 1254-byte corpus", file=sys.stderr)
    raise SystemExit(1)
...
completion = b"World protocol post-test timer: online\n"
...
transcript = output + "\n" + error
hex_matches = re.findall(r"(?m)^WORLD_PROTOCOL_V1_HEX=([0-9a-f]+)$", output)
...
required = [
    ...
    "World protocol Rust/.oc exact-byte convergence: PASS\n",
    "World protocol negotiation/canonical rejection: PASS\n",
    "World protocol bounded malformed rejection: PASS\n",
    "World protocol v1 native smoke: PASS\n",
    "World protocol post-test timer: online\n",
]
...
if not hex_valid and actual_hex:
    ...
    print(
        "World protocol corpus mismatch: "
...
if (
    ...
    print("World protocol v1 QEMU smoke: FAIL", file=sys.stderr)
    ...
    if not ordered:
        print("World protocol marker order is invalid", file=sys.stderr)
    if not timer_valid:
...
print(output, end="")
print(
    "World protocol exact corpus: "
    f"{len(expected_hex) // 2} bytes / {len(expected_hex)} lowercase hex digits"
)
print("World protocol v1 QEMU smoke: PASS")
PY
